class PCC {
	...
	none() {}
	clear() {return (!this.debug) ? this.none() : console.clear.apply(this, arguments);}
	count() {return (!this.debug) ? this.none() : console.count.apply(this, arguments);}
	debug() {return (!this.debug) ? this.none() : console.debug.apply(this, arguments);}
	error() {return (!this.debug) ? this.none() : console.error.apply(this, arguments);}
	group() {return (!this.debug) ? this.none() : console.group.apply(this, arguments);}
	groupCollapsed() {return (!this.debug) ? this.none() : console.groupCollapsed.apply(this, arguments);}
	groupEnd() {return (!this.debug) ? this.none() : console.groupEnd.apply(this, arguments);}
	info() {return (!this.debug) ? this.none() : console.info.apply(this, arguments);}
	log() {return (!this.debug) ? this.none() : console.log.apply(this, arguments);}
	markTimeline() {return (!this.debug) ? this.none() : console.markTimeline.apply(this, arguments);}
	profile() {return (!this.debug) ? this.none() : console.profile.apply(this, arguments);}
	profileEnd() {return (!this.debug) ? this.none() : console.profileEnd.apply(this, arguments);}
	table() {return (!this.debug) ? this.none() : console.table.apply(this, arguments);}
	time() {return (!this.debug) ? this.none() : console.time.apply(this, arguments);}
	timeEnd() {return (!this.debug) ? this.none() : console.timeEnd.apply(this, arguments);}
	timeStamp() {return (!this.debug) ? this.none() : console.timeStamp.apply(this, arguments);}
	timeline() {return (!this.debug) ? this.none() : console.timeline.apply(this, arguments);}
	timelineEnd() {return (!this.debug) ? this.none() : console.timelineEnd.apply(this, arguments);}
	trace() {return (!this.debug) ? this.none() : console.trace.apply(this, arguments);}
	warn() {return (!this.debug) ? this.none() : console.warn.apply(this, arguments);}
	test() {return console.log('this is a test');}
}
...
function applyDefaultActions(e, lookInsideElement, addBack)
{
	...
	if(tmp.length>0)
	{
		tmp.mCustomScrollbar({theme:"minimal-dark", scrollInertia:300});
	}
	...
	if(inElement.find('[data-action=mark-all]').length > 0)
	{
		...
		.on('checkAllMarked', function(){
			var checkBoxes = $(this).parent().siblings('.col-main').find('input[type=checkbox]');
	...
	inElement.findIn('a.xld,.xld:not(a,form,button,[disabled],.disabled) a:not(.normal),.main-menu a:not(.normal)', addBack).each(function (){
		...
		e(this).off('click').ajaxify({
			...
			onSuccess:function(op){
				...
				//var cw = e(op.target);
				if(cw.hasClass('mCustomScrollbar')==false){
					cw.mCustomScrollbar({theme:"minimal-dark", scrollInertia:100,mouseWheel:{preventDefault:true,deltaFactor:10}});
				}
				e('.xmodal').modal('hide');
...
function collapseSideBar(transition,dontSaveState) {
	...
	if(typeof $.fn.mCustomScrollbar=='function' && $('.left-sidebar').hasClass('mCustomScrollbar'))
	{
		$('.left-sidebar').mCustomScrollbar('destroy');
	}
...
function expandSideBar(noTransition,dontSaveState) {
	...
	if(typeof $.fn.mCustomScrollbar=='function' && !$('.left-sidebar').hasClass('mCustomScrollbar'))
	{
		$('.left-sidebar').mCustomScrollbar({theme:"minimal-dark", scrollInertia:300});
	}
...
$(document).ready(function(){
	...
	//
	$.ajax({
		url: ROOT_PATH+'/agent/widget/getList/'+Math.random()+'r',
		type: 'POST',
		dataType: 'json',
		cache: false,
	...
	window.fixWrapperIfram = function()
	{
		...
		if(cwiframe.length>0)
		{
			cw.mCustomScrollbar('destroy');
		}
	...
	if(typeof $.fn.mCustomScrollbar=='function')
	{
		if(!$('.left-sidebar').hasClass('mCustomScrollbar'))
			$('.left-sidebar').mCustomScrollbar({theme:"minimal-dark", scrollInertia:300});
		/**/
		if($('.container .row .content-wrapper iframe.pcciframe').length==0)
		{
			$('.container .row .content-wrapper').mCustomScrollbar({theme:"minimal-dark", scrollInertia:100,mouseWheel:{preventDefault:true,deltaFactor:10}});
		}
		//fixWrapperIfram();
	}
...
$.fn.loadWidget = function(cat, type, plugin, widget, params,dataType, wuid)
{
	...
	setTimeout(function(){
		var ddd = Date.now()
		$.ajax({
			url: ROOT_PATH+'/agent/widget/getContent/'+Math.random()+'r',
			type: 'POST',
...
_ = function(){
	...
	switch(arguments.length)
	{
		...
		{
			if(typeof AppLocales[_domain].LC_MESSAGES[arguments[0]]!='undefined')
			{
				return AppLocales[_domain].LC_MESSAGES[arguments[0]];
			}
		}
		return arguments[0];
		case 2:
		return (typeof AppLocales[arguments[0]].LC_MESSAGES[arguments[1]]!='undefined') ? AppLocales[arguments[0]].LC_MESSAGES[arguments[1]] : arguments[1];

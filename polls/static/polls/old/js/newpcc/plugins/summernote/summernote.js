...
(function (factory) {
  ...
  var Renderer = function () {
    ...
    tplStatusbar = function (options) {
      if(options.disableResizebar)
        return '';
    ...
    this.createLayout = function ($holder, options) {
      ...
      if (options.height > 0) {
        $('<div class="note-statusbar">' + tplStatusbar(options) + '</div>').prependTo($editor);
      }
  ...
  $.fn.extend({
     ...
     * @returns {this}
     */
    summernoteResize: function(w,h)
    {
      if (this.length === 0) return;
      this.each(function (idx, elHolder) {
        var $holder = $(elHolder);
        var $editor = $holder.siblings('.note-editor');
        $editor.width(w);
        $editor.find('.note-editable').height(h);
      });
    },
